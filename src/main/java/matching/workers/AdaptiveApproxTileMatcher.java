...
public class AdaptiveApproxTileMatcher<S> extends TileMatcher<S> {
    ...
    private void init(Collection<? extends MosaicTile<S>> data) {
        ...
        mCurrentColorApprox = mData.keySet().iterator().next(); // init with one valid value
    }
    ...
    protected Optional<MosaicTile<S>> calculateBestMatch(MosaicFragment fragment) {
        ...
        return Optional.ofNullable(mData.get(currentColorApprox));
    }
    ...
    public boolean removeTile(MosaicTile<S> toRemove) {
        boolean hadTile = mData.containsKey(toRemove.getAverageARGB());
    ...
    public void setColorMetric(ColorMetric metric) {
        ...
        super.setColorMetric(metric);
        List<MosaicTile<S>> tiles = new ArrayList<>(mData.values());
        init(tiles);
    }
}
